...
function addTodo(title: string, description?: string): string {
  const todo: ToDo = {
    id: array.length + 1,
    title,
    createdOn: new Date().toISOString().split("T")[0] as ISODate,
    completed: false,
    completedOn: null,
  };
...
async function main() {
  ...
  do {
    ...
    if (choice === 1) {
      let title: string = await rl.question("tite: ");
      let description: string = await rl.question("desc: ");
